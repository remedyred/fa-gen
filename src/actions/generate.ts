...
export default async function() {
	...
	async function processIcon(raw_icon_name) {
		// get the icon object
		const icon = parseIcon(raw_icon_name)
		...
		const import_string = getImportString(icon)
	...
	for (const raw_icon_name of config.icons) {
		await processIcon(raw_icon_name)
	}

	for (const [alias, resolved_icon] of Object.entries(config.aliases)) {
		const matching_icons = icons.filter(icon => icon.name === resolved_icon || icon.id === resolved_icon)
	...
	const alias_count = Object.keys(aliases).length
	...
	if (content.length) {
		...
		const contentString = getStringContent(content, config)

		mkdir(path.dirname(boot_path), true)
		...
		saveFile(boot_path, contentItems.join('\n'))
		const newConfig = {
			...config,
